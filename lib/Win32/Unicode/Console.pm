...
sub _ConsoleOut {
    ...
    unless (is_console($handle)) {
        # For non-console output (redirected, piped, or captured),
        # encode UTF-8 flagged strings to UTF-8 bytes for Windows 11 compatibility
        require Encode;
        my @output = map {
            defined($_) && utf8::is_utf8($_) ? Encode::encode_utf8($_) : $_;
        } @_;
        ...
        if ($handle == $STD_HANDLE->{&STD_ERROR_HANDLE}) {
            if (ref tied *STDERR eq 'Win32::Unicode::Console::Tie') {
                ...
                untie *STDERR;
                binmode STDERR, ':raw';
                my $ret = print STDERR @output;
                tie *STDERR, 'Win32::Unicode::Console::Tie';
                return $ret;
            }
            binmode STDERR, ':raw';
            return print STDERR @output;
        }
